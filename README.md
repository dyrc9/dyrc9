...
The structure contract lives in [`portfolio.schema.json`](./portfolio.schema.json). The dependency-free validator enforces its required fields, types, minimum lengths and item counts, unique array items, enums, patterns, URI formats, and declared properties. It also fails closed if the schema adopts an unsupported keyword, type, format, or external reference, so a future schema edit cannot silently promise a constraint the local validator ignores. It then applies repository-specific rules such as owner/repo URL consistency, mutually exclusive active/supporting repository classification, shipped-workflow references and CLI surface claims that include a declared lowercase command token, evidence slices for every runtime and workflow CLI, complete operator docs for workflow CLIs, unique examples, and standalone quickstart/proof commands that invoke a declared command on the documented product CLI without shell chaining, redirection, command substitution, or embedded newlines.
